...
func New(cfg *config.Config, database *db.DB, logger *slog.Logger) http.Handler {
	...
	// ── CORS origins
	// Always allow the configured frontend URL and the API's own base URL.
	// Additional origins come from CORS_ALLOWED_ORIGINS env var.
	// In development, "null" is included so the HTML test console works when
	// opened directly from the filesystem (file:// → origin: null).
	corsOrigins := []string{
		"https://*",
		"http://localhost:*",
		"http://127.0.0.1:*",
	...
	corsOrigins = append(corsOrigins, cfg.CORSOrigins...)
	if cfg.Env == "development" {
		corsOrigins = append(corsOrigins, "null") // file:// origin when HTML opened from disk
	}
	...
	r.Use(cors.Handler(cors.Options{
		...
		AllowCredentials: false, // must be false when AllowedOrigins contains wildcard or "null"
		MaxAge:           300,
	}))
