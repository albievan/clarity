...
// Handler holds the HTTP handler functions for the budgetperiods domain.
type Handler struct {
	svc Service
}
...
// Get handles GET /budgetperiods/{id}.
...
// Create handles POST /budgetperiods.
...
// Update handles PUT /budgetperiods/{id}.
...
// Delete handles DELETE /budgetperiods/{id}.
...
// Reopen handles the Reopen action.
func (h *Handler) Reopen(w http.ResponseWriter, r *http.Request) {
	// TODO: implement Reopen
	response.JSON(w, http.StatusNotImplemented, map[string]string{"message": "Reopen not yet implemented"})
}
