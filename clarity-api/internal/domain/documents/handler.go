...
// Handler holds the HTTP handler functions for the documents domain.
type Handler struct {
	svc Service
}
...
// GetDownloadURL handles the GetDownloadURL action.
func (h *Handler) GetDownloadURL(w http.ResponseWriter, r *http.Request) {
	// TODO: implement GetDownloadURL
	response.JSON(w, http.StatusNotImplemented, map[string]string{"message": "GetDownloadURL not yet implemented"})
}
